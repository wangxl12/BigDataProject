...
def productsClassID_statistics(df, top_n=5, prod_clss=1000):
    """统计各类商品在四个行为中的数量
    ...
    for prod_clss_id, _ in df['商品类目ID'].value_counts()[: prod_clss].items():
...
def productsID_statisctis(df, top_n=5):
    """统计各件商品在四个行为中的排名
